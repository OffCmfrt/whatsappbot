...
-- 11. Store shoppers: Composite index for self-join deduplication queries
-- Optimizes: SELECT order_id, MAX(updated_at) FROM store_shoppers GROUP BY order_id
-- This is the PRIMARY cause of 800ms+ slow queries in admin shoppers/confirmed endpoints
CREATE INDEX IF NOT EXISTS idx_shoppers_order_updated 
ON store_shoppers(order_id, updated_at DESC);

-- 12. Store shoppers: Status index for confirmed shoppers query
-- Optimizes: WHERE status = 'confirmed' in the self-join subquery
CREATE INDEX IF NOT EXISTS idx_shoppers_status_updated 
ON store_shoppers(status, updated_at DESC);
